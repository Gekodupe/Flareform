#!/usr/bin/env node
// Minimal static file server for local Flareform preview
...
const BLOCKED_PREFIXES = [
  ...
  path.join(ROOT, 'worker') + path.sep,
...
http.createServer((req, res) => {
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    const ext = path.extname(filePath).toLowerCase();
    const cacheable = ['.js', '.css', '.svg', '.png', '.ico', '.woff2'];
    const headers = Object.assign({}, SECURITY_HEADERS, {
  ...
  process.stdout.write('Flareform static server http://127.0.0.1:' + PORT + '\n');
});
