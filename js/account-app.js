...
function acctLoadSession() {
  ...
  if (typeof fbSaveSession === 'function') fbSaveSession(acctSession, acctEmail);
}
...
function acctRenderSignedIn(account) {
  ...
  if (planEl) {
    ...
    var line = plan;
    if (paid && status && status !== 'active' && status !== 'none') line += ' · ' + status.replace(/_/g, ' ');
    if (!paid) line += ' · Free tier';
  ...
  if (usageEl && account && account.usage) {
    usageEl.textContent =
      (account.usage.projects || 0) +
      '/' +
      (account.usage.maxProjects || 0) +
      ' projects · ' +
      (account.usage.submissions30d || 0) +
      '/' +
      (account.usage.submissionsLimit || 0) +
      ' submissions (30d)';
  } else if (usageEl) {
...
async function acctRegister() {
  ...
  try {
    ...
    showToast(res.emailSent === false ? (res.message || 'Account created') : 'Account created', res.emailSent === false ? 'warning' : 'success');
  } catch (e) {
...
async function acctStartSignIn() {
  ...
  try {
    ...
    showToast('Sign-in email sent', 'success');
  } catch (e) {
